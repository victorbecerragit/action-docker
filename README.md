...
- [Kubernetes — Latest News, Reports & Analysis](https://thehackernews.com/search/label/Kubernetes) — A high-severity security vulnerability has been disclosed in Docker Engine that could permit an attacker to bypass authorization plugins ( AuthZ ) under ...
- [kubernetes-kube-apiserver Vulnerabilities (Continuously ...](https://images.chainguard.dev/directory/image/kubernetes-kube-apiserver/vulnerabilities) — Chainguard Containers are rebuilt daily from source and rapidly patched against new vulnerabilities. Looking for a status of a CVE?
<!-- KUBE_CVEs_END -->
